# 17-1. Other Languages: Modify the API call in python_repos.py so it generates a chart,
...
# make a bar chart showing the most active discussions currently happening on Hacker News .
# The height of each bar should correspond to the number of comments each submission has .
# The label for each bar should include the submission’s title,
...
# Write a program called test_python_repos.py, which uses unittest to assert that the value of status_code is 200 .
# Figure out some other assertions you can make—for example,
# that the number of items returned is expected and,
